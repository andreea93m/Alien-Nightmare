...
void Model3ds::renderMeshImpl(Lib3dsMeshInstanceNode *node, Lib3dsMesh *mesh, RenderOptions options) {
	...
	//do it the same way as in bounding box calculation (see file_bounding_box_of_nodes_impl), this should hopefully be correct
	float inv_matrix[4][4];
	lib3ds_matrix_copy(inv_matrix, mesh->matrix);
	lib3ds_matrix_inv(inv_matrix);
	glMultMatrixf(&inv_matrix[0][0]);

	LOG_DEBUG << fileName << " mesh-matrix " << std::endl
		<< mesh->matrix[0][0] << " " << mesh->matrix[0][1] << " " << mesh->matrix[0][2] << " " << mesh->matrix[0][3] << std::endl
		<< mesh->matrix[1][0] << " " << mesh->matrix[1][1] << " " << mesh->matrix[1][2] << " " << mesh->matrix[1][3] << std::endl
		<< mesh->matrix[2][0] << " " << mesh->matrix[2][1] << " " << mesh->matrix[2][2] << " " << mesh->matrix[2][3] << std::endl
		<< mesh->matrix[3][0] << " " << mesh->matrix[3][1] << " " << mesh->matrix[3][2] << " " << mesh->matrix[3][3] << std::endl;
